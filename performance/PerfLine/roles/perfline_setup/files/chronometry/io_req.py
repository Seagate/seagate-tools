...
def graph_build(relations, ext_graph: Digraph=None, is_cob: bool=False):
    ...
    if ext_graph is None:
        graph.render(filename="{}_graph_{}".format(t, relations[0]['client_id']))
...
fom_to_stio.stio_id,
tx_to_gr.gr_id,
client_to_{io_req_type}.pid,
fom_desc.pid

FROM client_to_{io_req_type}
JOIN {io_req_type}_to_rpc  on client_to_{io_req_type}.{io_req_type}_id={io_req_type}_to_rpc.{io_req_type}_id
...
{pid_filter}
AND   client_to_{io_req_type}.pid={io_req_type}_to_rpc.pid AND client_to_{io_req_type}.pid=rpc_to_sxid.pid
...
def get_timelines(client_id: str, grange: int, client_pid: int=None, create_attr_graph: bool=False,
    ...
    labels = ("crpc_id", "client_id", f"{io_req_type}_id", "opcode", "cxid",
    ...
    io_req_d = query2dlist(
        io_req_table.select().where(io_req_table.id==io_req_id) if client_pid is None else
        io_req_table.select().where((io_req_table.id==io_req_id)&(io_req_table.pid==client_pid)))
    ...
    if create_attr_graph:
        ...
        attr_graph = graph_build(relations, ext_graph, is_cob)
...
def parse_args():
    ...
    io_req.py: Display significant performance counters for Motr stack.
    """)
    ...
    parser.add_argument("-c", "--cob", action='store_true',
                        help="Whether client request relates to COB")
...
if __name__ == '__main__':
    ...
    time_table, queue_table, queue_start_time, client_start_time, _ = \
        get_timelines(args.client_id, args.grange, args.pid, args.attr, False, None, args.cob)
