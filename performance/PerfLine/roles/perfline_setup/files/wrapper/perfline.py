...
sys.path.insert(1, os.path.join(os.getcwd(), 'core'))
...
def args_parse():
    description="""
    perfline.py: front-end module for cluster usage

    perfline.py implements persistent queue and enqueues
    performance tests requests on the hardware cluster.
    It takes YAML configuration file as input and supplies
    worker.sh script with configuration overrides,
    ...
    Task is added by calling perfline.py with '-a'/'--add-task'
    ...
    perfline.py supports task priorities. If priority is given
    in YAML configuration file, perfline will enqueue task
    ...
    is created and enqueued. perfline returns TID of the
    newly created task.
    ...
    group.add_argument('-a', '--add-task',
                       help="Add task to the queue",
                       action="store_true")
    group.add_argument('-d', '--del-task',
                       metavar='task_id',
                       help="Delete task from the queue")
