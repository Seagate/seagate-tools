...
show_usage() {
        ...
        echo -e "\tExample\t:\t ./s3cosbench_benchmark.sh -nc \"8,16,32\" -ns \"1024,2048,4096 \" -s \"1Mb,4Mb,16Mb\"  -b 1,10,50 -w mixed -t 600  \n"
        exit 1
}
      ...
      do
          ...
          do
               ...
               do
                    ...
                    check_completion `tail -n 3 benchmark.log/workloads | grep Accepted | cut -d ":" -f2 | tr -d ' '`
                    echo "Cosbench Triggered for worker: $clients sample: $sample obj_size:$size bucket:$bucket"
