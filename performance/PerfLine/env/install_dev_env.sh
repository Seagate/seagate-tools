...
PERFLINE_DEFAULT_REPO="https://github.com/Seagate/seagate-tools.git"
PERFLINE_DEFAULT_BRANCH="main"
...
SERVICE_FRAMEWORK_DISK_DEFAULT="/dev/sdb"

SERVICE_FRAMEWORK_DIR="/root/deploy-scripts"
...
set +x
	...
	if [ "$SERVICE_FRAMEWORK_DISK" = "null" ] ; then
	    SERVICE_FRAMEWORK_DISK="$SERVICE_FRAMEWORK_DISK_DEFAULT"
	fi
...
function deploy_cluster() {
    ...
    if [ -f "$SERVICE_FRAMEWORK_DIR"/k8_cortx_cloud/deploy-cortx-cloud.sh ] ; then
	set +e
	pushd "$SERVICE_FRAMEWORK_DIR"/k8_cortx_cloud
    ...
    pushd cortx-re/solutions/kubernetes
    export CORTX_SCRIPTS_REPO="$CORTX_SERVICE_REPO" && export CORTX_SCRIPTS_BRANCH="$CORTX_SERVICE_BRANCH" && export SNS_CONFIG="$SNS_LAYOUT" && export DIX_CONFIG="$DIX_LAYOUT" && export SOLUTION_CONFIG_TYPE="$CONFIG_TYPE" && export SYSTEM_DRIVE="$SERVICE_FRAMEWORK_DISK" && ./cortx-deploy.sh --cortx-cluster
...
function install_perfline() {
	...
	idx=$((i-1))
	host=$(yq_linux_386 ".specification.nodes[$idx].hostname" "$SPEC_FILE")
	ssh "$host" "systemctl stop puppet"
	ssh "$host" "systemctl disable puppet"
    done
    ...
    sed -i 's:^ha_type=.*:ha_type=hare:g' "$PERFLINE_HOSTS_FILENAME"
    sed -i "s:^cortx_k8s_repo=.*:cortx_k8s_repo=$SERVICE_FRAMEWORK_DIR:g" "$PERFLINE_HOSTS_FILENAME"
    sed -i "s/^cluster_pass=.*/cluster_pass=$pass/g" "$PERFLINE_HOSTS_FILENAME"

    sed -i "s:^disk=.*:disk=$SERVICE_FRAMEWORK_DISK:g" "$PERFLINE_HOSTS_FILENAME"
