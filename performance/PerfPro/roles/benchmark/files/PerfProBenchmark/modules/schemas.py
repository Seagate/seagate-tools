...
def get_degraded_IO_primary_set():
   return {**get_s3bench_primary_set(),
            **{
            "cluster_state": "",
            }
...
def set_degraded_IO_set(self,operation):
    primary_set = get_degarded_IO_primary_set(operation)
    primary_set["Name"] = self.bench
    primary_set["Build"] = self.Build
    primary_set["Version"] = self.Version
    primary_set["Branch"] = self.Branch
    ...
    primary_set["Sessions"] = self.sessions
    primary_set["Operation"] = operation
    primary_set["cluster_state"] = self.cluster_state

    return primary_set


def get_degraded_IO_insertion_schema():
    return {**get_degraded_IO_primary_set(),
            **{
            "Timestamp": "",
            "Host": "",
            "Log_File": ""
