...
function save_s3_configs() {
    ...
    for srv in $(echo $NODES | tr ',' ' '); do
        pod=$(ssh "$PRIMARY_NODE" "kubectl get pods -n $NAMESPACE -o wide" | grep "$srv" | grep "$CORTX_SERVER_POD_PREFIX" | awk '{print $1}')
        ...
        for cont in $containers ; do
            ...
            # Copy config
            ssh "$PRIMARY_NODE" "kubectl exec -n $NAMESPACE $pod -c $cont -- cat /etc/cortx/s3/conf/s3config.yaml" > s3config.yaml
            ssh "$PRIMARY_NODE" "kubectl exec -n $NAMESPACE $pod -c $cont -- cat /opt/seagate/cortx/s3/s3startsystem.sh" > s3startsystem.sh
            ssh "$PRIMARY_NODE" "kubectl exec -n $NAMESPACE $pod -c $cont -- cat /etc/cortx/cluster.conf" > cluster.conf
            ssh "$PRIMARY_NODE" "kubectl exec -n $NAMESPACE $pod -c $cont -- cat /etc/hosts" > hosts
            ssh "$PRIMARY_NODE" "kubectl exec -n $NAMESPACE $pod -c $cont -- cat /etc/haproxy/haproxy.cfg" > haproxy.cfg
            ...
            echo ssh "$PRIMARY_NODE" "kubectl exec -n $NAMESPACE $pod -c $cont -- ps auxww"

            # Gather process FID
            service=$(ssh "$PRIMARY_NODE" "kubectl exec -n $NAMESPACE $pod -c $cont -- ps auxww" | grep s3server | grep motr | tr ' ' '\n' | grep -A1 "processfid" | tail -n1)
        ...
        for cont in $containers ; do
            mkdir -p "$cont"
            pushd "$cont"
            ssh "$PRIMARY_NODE" "kubectl exec -n $NAMESPACE $pod -c $cont -- cat /etc/cortx/s3/haproxy.cfg" > haproxy.cfg
            popd
        done
