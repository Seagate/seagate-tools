...
def graph_build(m_relations, relations, ext_graph: Digraph=None):
...
client_to_dix.client_id,
client_to_dix.dix_id,
{}
dix_to_cas.cas_id,
rpc_to_sxid.opcode,
...
{}
AND   client_to_dix.pid=dix_to_cas.pid
AND   client_to_dix.pid=cas_to_rpc.pid AND client_to_dix.pid=rpc_to_sxid.pid
...
AND   client_to_dix.client_id={};
"""

def update_tables_common(relations, grange: int, time_table, client_start_time):
        ...
        if tx_id:
            ...
            if grange[0]>0:
                left  = query2dlist(fom_req.select().where((fom_req.id==gr_id)&
                                                           (fom_req.pid==srv_pid)&
                                                           (fom_req.time<client_start_time)).limit(grange[0]))
            if grange[1]>0:
                right = query2dlist(fom_req.select().where((fom_req.id==gr_id)&
                                                           (fom_req.pid==srv_pid)&
                                                           (fom_req.time>=client_start_time)).limit(grange[1]))
...
def get_timelines(client_id: str, grange: int, client_pid: int=None, create_attr_graph: bool=False,
    ...
    pid_filter = f"AND   client_to_dix.pid={client_pid}" if client_pid is not None else ""
    query  = query_gen.format("", "", "client_to_dix.dix_id", pid_filter, client_id)
    mquery = query_gen.format("dix_to_mdix.mdix_id,",
                              "JOIN dix_to_mdix on client_to_dix.dix_id=dix_to_mdix.dix_id",
                              "dix_to_mdix.mdix_id", pid_filter, client_id)
    ...
    labels = ("crpc_id", "client_id", "dix_id", "cas_id", "opcode",
    ...
    client_req_d = query2dlist(
        client_req.select().where(client_req.id==client_id) if client_pid is None else
        client_req.select().where((client_req.id==client_id)&(client_req.pid==client_pid)))
    times_tag_append(client_req_d, 'op', f"client {client_id}")
    ...
    if m_relations:
        m_dix_req_d = query2dlist(
            dix_req.select().where(dix_req.id==m_dix_id) if client_pid is None else
            dix_req.select().where((dix_req.id==m_dix_id)&(dix_req.pid==client_pid)))
    ...
    if create_attr_graph:
        print("Building attributes graph...")
        attr_graph = graph_build(m_relations, relations, ext_graph)

    return time_table, queue_table, queue_start_time, client_start_time, attr_graph

def parse_args():
    parser = argparse.ArgumentParser(prog=sys.argv[0], description="""
    md_req.py: Display significant performance counters for MD Motr stack.
    """)
...
if __name__ == '__main__':
    ...
    time_table, queue_table, queue_start_time, client_start_time, _ = \
    ...
    draw_timelines(time_table, queue_table, client_start_time, queue_start_time,
