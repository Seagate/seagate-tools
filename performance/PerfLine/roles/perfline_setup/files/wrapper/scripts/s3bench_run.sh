...
function run_s3bench()
{
    ...
    if [[ -f "s3bench_report.csv" ]]; then
        $PERFLINE_DIR/bin/s3bench_perfline -reportFormat "$format" -accessKey $ACCESS_KEY -accessSecret $SECRET_KEY -bucket $BUCKET_NAME -numSamples $NUM_SAMPLES -objectSize $OBJ_SIZE -numClients $NUM_CLIENTS -endpoint "$ENDPOINT" -o s3bench_report.csv -t csv+
    else
        $PERFLINE_DIR/bin/s3bench_perfline -reportFormat "$format" -accessKey $ACCESS_KEY -accessSecret $SECRET_KEY -bucket $BUCKET_NAME -numSamples $NUM_SAMPLES -objectSize $OBJ_SIZE -numClients $NUM_CLIENTS -endpoint "$ENDPOINT" -o s3bench_report.csv -t csv
    fi
